...
func (s *State) ExplorerSaveFile(fileType, ext, defaultFileName string) (path string, err error) {
	...
	buf := make([]uint16, w32.MAX_PATH)
	if defaultFileName != "" {
		nameUTF16, convErr := syscall.UTF16FromString(defaultFileName)
		if convErr != nil {
			return "", fmt.Errorf("failed to encode default filename: %w", convErr)
		}
		copy(buf, nameUTF16)
	}
	filter := unsafe.SliceData(utf16.Encode([]rune(fileType + " (" + ext + ")\000" + ext + "\000\000")))
	title, err := syscall.UTF16PtrFromString(lang.LocalizeKey("common.save_file", "Save {{.FileType}}", map[string]any{"FileType": fileType}))
	if err != nil {
		return "", err
	}
	ofn := &w32.OPENFILENAME{
		...
		File:    unsafe.SliceData(buf),
		MaxFile: uint32(len(buf)),
		Flags:   w32.OFN_OVERWRITEPROMPT | w32.OFN_PATHMUSTEXIST | w32.OFN_NOCHANGEDIR,
		Filter:  filter,
		Title:   title,
	}
	ofn.StructSize = uint32(unsafe.Sizeof(*ofn))
	if !w32.GetSaveFileName(ofn) {
		return "", errors.New("file save cancelled")
	}
	i := 0
	for i < len(buf) && buf[i] != 0 {
		i++
