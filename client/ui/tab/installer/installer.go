...
func NewInstallerTab(s *uicommon.State) uicommon.Tab {
	...
	i = Installer{
		...
		installButton:    widget.NewButtonWithIcon(lang.LocalizeKey("installer.install", "インストール"), theme.DownloadIcon(), nil),
...
// 		if err != nil {
// 			i.state.ErrorText.Segments = []widget.RichTextSegment{
// 				&widget.TextSegment{Text: lang.LocalizeKey("installer.error.failed_to_get_mods", "Modの取得に失敗しました。"), Style: widget.RichTextStyle{ColorName: theme.ColorNameError}},
// 			}
// 			i.state.ErrorText.Refresh()
// 			i.state.ErrorText.Show()
// 			return
...
// 			if !ok {
// 				i.state.ErrorText.Segments = []widget.RichTextSegment{
// 					&widget.TextSegment{Text: lang.LocalizeKey("installer.error.mod_not_found_in_pack", "Modパック内のModが見つかりません: ") + modID, Style: widget.RichTextStyle{ColorName: theme.ColorNameError}},
// 				}
// 				i.state.ErrorText.Refresh()
// 				i.state.ErrorText.Show()
// 				return
...
// 			i.state.ErrorText.Segments = []widget.RichTextSegment{
// 				&widget.TextSegment{Text: lang.LocalizeKey("installer.error.dependency_conflict", "Modの依存関係に競合があります: ") + strings.Join(conflictMods, ", "), Style: widget.RichTextStyle{ColorName: theme.ColorNameError}},
// 			}
// 			i.state.ErrorText.Refresh()
// 			i.state.ErrorText.Show()
// 			return
...
// 	if err != nil {
// 		i.state.ErrorText.Segments = []widget.RichTextSegment{
// 			&widget.TextSegment{Text: lang.LocalizeKey("installer.error.binary_not_found", "指定されたパスからAmong Usの実行ファイルが見つかりません。"), Style: widget.RichTextStyle{ColorName: theme.ColorNameError}},
// 		}
// 		i.state.ErrorText.Refresh()
// 		i.state.ErrorText.Show()
// 		return
...
// 	if _, ok := resolved[mod.ID]; ok {
// 		return nil
// 	}
// 	if _, ok := unresolved[mod.ID]; ok {
// 		return fmt.Errorf("circular dependency detected: %s", mod.ID)
// 	}
// 	unresolved[mod.ID] = struct{}{}
// 	for _, dep := range mod.Dependencies {
// 		switch dep.Type {
// 		case modmgr.ModDependencyTypeRequired:
// 			depMod, err := i.state.Mod(dep.ID)
// 			if err != nil {
// 				return fmt.Errorf("failed to resolve dependency %s for mod %s: %w", dep.ID, mod.ID, err)
// 			}
...
// 			depMod, err := i.state.Mod(dep.ID)
// 			if err != nil {
// 				slog.Info("Optional dependency not found, skipping", "dependency", dep.ID, "mod", mod.ID)
// 				continue
// 			}
// 			if err := i.resolveDependencies(*depMod, resolved, unresolved, conflict); err != nil {
// 				return err
...
// 			conflict[mod.ID] = append(conflict[mod.ID], dep.ID)
...
func (i *Installer) refreshModInstallation() {
	...
	if ok, err := i.state.ModInstalled.Get(); ok && err == nil {
		...
		for _, mod := range installationInfo.InstalledMods {
			// if mod.Type == modmgr.ModTypeModPack {
			// 	continue
			// }
			if idx > 0 {
				modNames += ", "
			}
			modNames += mod.ID + " (" + mod.ID + ")"
			idx++
		}
